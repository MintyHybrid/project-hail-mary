...
subtitle: "A catalytic nucleic acid motif sequence conserved across Potyviridae and eukaryotic genes"
...
<!-- NOTE: the high-level "Executive summary" bullets now live in the book overview (index.qmd). -->

## Rationale and roadmap {#sec-roadmap}

To make the reasoning followable — and to be explicit about where it is speculation
versus supported — here is the actual chain of thought, step by step:

0. **be curious and wonder ...** "Evolution is extremely good at filling every nook in the ecosystem." (Andy Weir) - do biological systems maybe "use" DNAzymes?
1. **Start from a real DNAzyme.** I BLASTed the **catalytic core of the 10-23
   deoxyribozyme** [@borggraefe2022] against the viral nucleotide collection and
   recovered **16 potyvirus isolates** (6 species) carrying that core.
2. **Ask whether it could be *active*.** *If* the core were catalytically active,
   it would cleave whatever RNA is **complementary to the two flanking oligomers**
   (the arms that give a 10-23 DNAzyme its target specificity). So I searched
   **eukaryotic** genomes for **perfect matches to the 9 + 9 nt flanks** of all 16
   isolates — i.e. for the sequences whose transcripts a natural version of this
   DNAzyme might target.
3. **The flank-matches hinted at an ecological pattern.** A quick look at two flanks
   (from a *Turnip mosaic virus* and a *Maize dwarf mosaic virus* isolate) suggested
   the recovered species might be **ecologically associated** (as it appeared to be enriched in plant and insect species), which made me
   speculate the viral↔eukaryote matches could be signatures of **horizontal gene
   transfer (HGT) to vectors and/or hosts** (if you are into computing rather than biology, think "evolutionary prompt injection".
4. **Caveat — this is not yet established.** It is *not* shown that the
   flank-matching species actually **are hosts or vectors** of the corresponding
   potyvirus. But I built the **BioNexus** app (Chapter 7) to test connectivity between the eukaryotic hits; the interactions (only interactions that have been documented) looked **sporadic rather than continuous** — which is *not* enough
   to infer the **absence** of HGT, either.
5. **A small aside.** A central target nucleotide was initially omitted; adding it
   back (as **G**) gives exactly one isolate a **single perfect match to *Aphis
   gossypii*** (a principal aphid vector) — a nice, suggestive observation, **not**
   strong evidence.
6. **The locus is the CI gene.** The DNAzyme-like sequence maps to the coding region
   of the potyviral **cylindrical inclusion (CI)** protein (Chapters 2–4).

**Where this goes.** Assessing potential DNAzyme catalytic activity (Chapter 3) 
shows most Potyvirus homologs carry **substitutions at activity-critical core positions**, 
so DNAzyme activity is **unlikely / rare** — and would additionally require reverse
transcription to coincide (Potyviridae genomes are based on +ssRNA - not DNA). The nucleotide-level analyses (logos, GC/CpG, codon
usage) are therefore **exploratory context, not evidence** and rather argue against in vivo DNAzyme activity and so does the even (negative) selection over the broader sequence context. The thread that eventually *does*
lead somewhere is the **CI protein itself** — specifically **host-type sorting
motifs** in the DNAzyme-locus region (a tyrosine-based motif adjacent to an acidic
dileucine signal, which has been characterized earlier [@wu2022]; Chapter 5). This putative signal connects to **AP-4-dependent trafficking and
the LE/MVB** and motivates the immune-trafficking hypothesis of Chapter 9.

## Intro

The starting point is a synthetic **10-23 deoxyribozyme** [@borggraefe2022]: a
single-stranded DNA able to cleave matching single-stranded RNA. The central catalytic domain of the DNAzyme comprises 15 nt - which are flanked by at least 9 nt 3' and 5'. The flanks, in turn represent the substrate-binding arms, i.e. they determine the specificity of the DNAzyme towards the RNA it is able to cleave. Complementary base-pairing with target RNA
(Watson–Crick), positions the 15-nt catalytic core that cleaves the RNA at an
unpaired purine·pyrimidine junction. 
...
region (below). *Scheme redrawn after Borggräfe et al. 2022 [@borggraefe2022];
...
this 15nt sequence against the viral nucleotide collection on NCBI and found that it
matches perfectly to a region in the genomes of several potyviruses.

The same day, I did a quick check for putative RNA targets (under the assumption that DNAzyme activity indeed occured), which sent me straight into the rabbit hole (probably for a bit too long). I picked two semi-random viral hits (maize and turnip mosaic virus). Semi-random, because if you would ask someone who knows me well enough to predict what I would be drawn to immediately had a pretty good chance of correctly guessing that: if there is something with a name indicating the virus infects a root crop, she will pick that one (turnip) and if she checked two of them and had already checked turnip, her second pick would not be another dicot infecting virus if there also was one targeting monocots (maize). That still leaves multiple isolates (blast hits) for each of these two viruses.  

These were my initial thoughts - which sound way too much like I had found the holy grail that I later realized I probably had not: 

### Implications part I

Now the crazy part - or rather the observation that got me hooked for the rest of the night and the following weeks. For these two examples (sequences based on) a Turnip mosaic
...
So much for my quick check, which took several hours to complete. That's how easy it is to fool me into months of analyses that I am not even paid to do...

The remainder of this notebook shows the systematic approach (and the code I wrote, so everyone can reproduce my fails or adapt it to other problems [at least, failures might happen a lot faster then]).
...
I had blasted the catalytic domain of the 10-23 DNAzyme characterised by
@borggraefe2022.
...
The BLAST algorithm and scoring are described in @altschul1997.
...
As my query was a short sequence (15mer) and the database is far from small, I had naively expected that I should get some hits by chance alone already, which I 
suspected could be more or less randomly distributed over a couple of possibly very
...
to the same genus: Potyvirus.
That's where I almost cut my fingers with Occam's Razor: Potyviruses are members of the family of Potyviridae, which in turn classify as Type IV group viruses according to the Baltimore classification (i.e. they are +ssRNA viruses). DNAzyme activity therefore is nothing, that could be expected directly.
...
## Potyvirus isolates with match to the DNAzyme catalytic center

In the code below, I load the document containing the full sequences (genomes, or partial genomes, respectively) corresponding to the 16 Blast hits. I had downloaded and saved the file locally. Since we are dealing with viruses, the file is not that huge. 
...
Next, I search the regions with the matches to the 15 bases I had blasted initially. I do
...
Since long (25+25bp) flanks did not yield any perfect hits, I crop down to 9bp
...
### Sequence logo of viral fragments
...
tree_line <- hailmary_palette()[["text_soft"]]
ft    <- ggtree(filtreedf, layout = "circular", colour = tree_line)
ftdl  <- ggtree(as.phylo(filtreefortified), layout = "daylight", colour = tree_line)
...
### closer inspection of eukaryotic hits
...
rc_G %>% ggplot(aes(y = query)) + geom_bar() + theme_manuscript() +
...
ggtree(df2, colour = hailmary_palette()[["text_soft"]]) +
  geom_tiplab(colour = hailmary_palette()[["text_strong"]]) +
  geom_nodelab(mapping = aes(label = labelinodes), colour = hailmary_palette()[["text_strong"]]) +
...
As those authors put it:

> "The pattern of occurrence of NRVSs and the phylogenetic analyses of NRVSs and
> related viruses indicate that multiple independent integrations into many plant
> lineages may have occurred."
>
> — @chiba2011

## Multilayer Network analysis

How I prepped for what went into the network-section (chapter 7) and the Nexus App.
...
initially planned to visualise taxonomies with the `metacoder` R package [@foster2017].
...
```{r}
#| echo: false
# Session info with absolute filesystem paths redacted (privacy: the raw output
# would otherwise print local library/tool paths containing the username).
